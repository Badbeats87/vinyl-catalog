 ...
 */
export async function searchReleasesCatalog(
  query: string,
  limit = 20
 ...
 */
export function formatSearchResults(results: SearchResult[]): SearchItem[] {
 ...
 */
export async function generateQuotesForItems(items: QuoteItem[]): Promise<QuoteResponse[]> {
  ...
  for (const item of items) {
    try {
      ...
      throw new Error(
        `Failed to generate quote for ${item.releaseId}: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
 ...
 */
export async function getSubmissionByNumber(submissionNumber: string): Promise<SubmissionDetail | null> {
