...
async function createPolicyFlow() {
    ...
    const scopeValue = scope !== 'global' ? await prompt(`${scope === 'genre' ? 'Genre' : 'Release ID'}: `) : undefined;
    const buyPercentageStr = await prompt('Buy Percentage (e.g., 0.55 for 55%) [0.55]: ') || '0.55';
    const sellPercentageStr = (await prompt('Sell Percentage (e.g., 1.25 for 125%) [1.25]: ')) || '1.25';
    try {
